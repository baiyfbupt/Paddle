...
inline void modulated_deformable_col2im_coord(
  ...
  int num_kernels = 2 * kernel_shape[2] * kernel_shape[3]
...
class ModulatedDeformableConvCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    output_4d.Resize(framework::make_ddim({batch_size / im2col_step, groups, M, N}));
    ...
    for (int i = 0; i < batch_size / im2col_step; i++) {
      modulated_deformable_im2col(ctx.device_context(),
